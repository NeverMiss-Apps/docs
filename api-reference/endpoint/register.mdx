...
api: "POST https://carteira.nevermissapps.com/v1/register"
description: "Configure chave PIX, saque automático, dados da empresa, webhook e callback URL"
---

## Descrição

Configura ou atualiza suas informações: chave PIX para saques, saque automático, dados da empresa (nome, logo e cor), webhook URL e callback URL. **Todos os campos são opcionais**.

<Note>
**Prefere interface visual?** Você pode configurar todas essas opções através do [Dashboard - Configurações](https://nevermissapps.com/dashboard/configuracoes) sem precisar usar a API.
</Note>
...
<ParamField body="companyColor" type="string">
  Cor principal da sua empresa em hexadecimal (ex: `#7C3AED`)
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="companyColor" icon="palette">
    Cor principal da sua empresa em formato hexadecimal.
    
    - Usada na página de pagamento para botões e elementos de destaque
    - Formato: `#RRGGBB` (ex: `#7C3AED`, `#10B981`)
    - Se não configurado, usa a cor padrão da plataforma
  </Accordion>
  ...
  <Accordion title="callbackUrl" icon="link">
    ...
    - Sistema gera: `https://carteira.nevermissapps.com/pay/{txid}`
...
```javascript
await fetch('https://carteira.nevermissapps.com/v1/register', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer SUA_API_KEY',
    'Content-Type': 'application/json'
