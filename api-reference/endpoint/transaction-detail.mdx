...
api: "GET https://carteira.nevermiss.com/v1/transactions/{txid}"
...
```bash cURL
curl -X GET https://carteira.nevermiss.com/v1/transactions/ABC123XYZ \
...
const response = await fetch(
  `https://carteira.nevermiss.com/v1/transactions/${txid}`,
...
response = requests.get(
    f'https://carteira.nevermiss.com/v1/transactions/{txid}',
    headers={'Authorization': 'Bearer SUA_API_KEY'}
)
...
<?php
$txid = 'ABC123XYZ';
$ch = curl_init("https://carteira.nevermiss.com/v1/transactions/$txid");
...
<AccordionGroup>
  ...
  <Accordion title="Acompanhar saque" icon="clock">
    ```javascript
    const { transaction } = await fetch(
      `https://carteira.nevermiss.com/v1/transactions/${withdrawalTxid}`,
      { headers: { 'Authorization': `Bearer ${API_KEY}` } }
    ).then(r => r.json());
